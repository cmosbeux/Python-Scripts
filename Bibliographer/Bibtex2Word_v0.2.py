...
#look for references in the document
for cite in txt.split(' '): #split text in words
    paperfound = False
    
    #make a round of papers
    for ref in papers.keys():    
    ...
    if firstauthor in txt:
        year = 0000
        indexes = [n for n in range(len(txt)) if txt.find(firstauthor, n) == n]
        #indices = [n for n, x in enumerate() if  txt.find(firstauthor, n)]    
    ...
    if paperfound:
        ...
        #check if cite appears more than once:
        # if new_line in new_lines.values():
        #     dummy = 1
        #     print('%s et al. (%s)' % (firstauthor, year))
        #     #repeats['%s et al. (%s)' % (firstauthor, year)] = repeats['%s et al. (%s)' % (firstauthor, year)] + 1
        # else:
        #     papercount+=1
        citecount+=1
